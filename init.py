...
def user_dialog(choicesvar, edit=False, user=None):
    ...
    if not edit:
        ok = Button(button_frame, text="Add",
                    command=lambda: user_edit_or_add(nickname_var.get(), ip_var.get(), key.get(1.0, 'end-1c'),
                                                     choicesvar))
    else:
        ...
        ok = Button(button_frame, text="Save",
                    command=lambda: user_edit_or_add(nickname_var.get(), ip_var.get(), key.get(1.0, 'end-1c'),
                                                     choicesvar, edit=True, nickname_before=user))
